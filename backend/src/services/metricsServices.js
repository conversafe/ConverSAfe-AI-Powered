...
export const getRoomMetrics = async (roomId, userId) => {
  ...
  if (room.admin._id.toString() !== userId.toString()) {
    const error = new Error("You must be admin of this room to see the metrics");
    error.statusCode = HTTP.STATUS.FORBIDDEN;
    throw error;
  }

  const messages = await Message.find({ chatRoom: roomId }).populate("sender", "name email").lean();
  ...
  const participantMetrics = room.participants.map(user => {
    ...
    const percentage = totalMessages > 0 ? ((count / totalMessages) * 100).toFixed(2) : "0.00";
